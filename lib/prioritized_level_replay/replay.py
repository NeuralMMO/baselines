...
class PrioritizedReplayDistribution:
  ...
  def __init__(
      ...
      # the beta coefficient for the score-prioritized distribution
  ) -> None:
  ...
  def create(
      ...
      last_episode_levels: Dict[
          Level, int
      ],  # the last episode that each level was played,
    ...
    last_episode_levels = torch.tensor(
        [v for v in last_episode_levels.values()])

    level_scores = torch.pow(
        input=F.normalize(score_levels, dim=-1), exponent=1 / self.temperature
    )
    ...
    ) * score_dist + self.staleness_coeff * stale_dist
...
class PrioritizedReplay:
  ...
  def sample_next_level(
    ...
    unseen_levels = [
        level for level in self.levels if level not in visited_levels]
