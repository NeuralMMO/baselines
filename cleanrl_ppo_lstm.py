...
class CleanPuffeRL:
  ...
  def resume_model(self, path):
    ...
    lib.agent.util.load_matching_state_dict(
        self.agent, resume_state["agent_state_dict"]
    )
  ...
  def allocate_storage(self):
    ...
    return SimpleNamespace(
        ...
        obs=torch.zeros(
            common_shape + self.binding.single_observation_space.shape
        ).to(self.device),
        actions=torch.zeros(
            common_shape + self.binding.single_action_space.shape, dtype=int
        ).to(self.device),
  ...
  def evaluate(self, agent, data, max_episodes=None):
    ...
    for step in range(0, self.num_steps + 1):
      ...
      for buf, envs in enumerate(self.buffers):
        ...
        if step == 0:
          ...
          data.rewards[step - 1, buf] = (
              torch.tensor(r).float().to(self.device).view(-1)
          )
          ...
          for item in i:
            if "episode" in item.keys():
              ...
              self.writer.add_scalar(
                  "charts/episodic_return", er, self.global_step
              )
              self.writer.add_scalar(
                  "charts/episodic_length", el, self.global_step
              )
              ...
              if "episode_stats" in agent_info.keys():
                ...
                for name, stat in agent_info["episode_stats"].items():
                  self.writer.add_histogram(
                      f"charts/episode_stats/{name}/hist",
                      stat,
                      self.global_step,
                  )
    ...
    self.writer.add_scalar(
        "performance/env_time",
        env_step_time,
        self.global_step)
    self.writer.add_scalar("performance/env_sps", env_sps, self.global_step)
    self.writer.add_scalar(
        "performance/inference_time", inference_time, self.global_step
    )
    self.writer.add_scalar(
        "performance/inference_sps", inference_sps, self.global_step
    )
    ...
    for profile in envs.profile():
      for k, v in profile.items():
        ...
        self.writer.add_scalar(
            f"performance/env/{k}", np.mean(v.delta), self.global_step
        )
  ...
  def train(
    ...
    assert (
        self.num_steps % bptt_horizon == 0
    ), "num_steps must be divisible by bptt_horizon"
    self.init_writer({})
    ...
    # flatten the batch
    b_obs = data.obs.reshape(
        (num_minibatches, bptt_horizon, -1)
        + self.binding.single_observation_space.shape
    )
    b_logprobs = data.logprobs.reshape(num_minibatches, bptt_horizon, -1)
    b_actions = data.actions.reshape(
        (num_minibatches, bptt_horizon, -1) + self.binding.single_action_space.shape)
    ...
    for epoch in range(update_epochs):
      ...
      for minibatch in range(num_minibatches):
        if self.agent.is_recurrent:
          ...
          initial_lstm_state = (
              initial_lstm_state[0].detach(),
              initial_lstm_state[1].detach(),
          )
        else:
          _, newlogprob, entropy, newvalue = agent.get_action_and_value(
              b_obs[minibatch].reshape(
                  -1, *self.binding.single_observation_space.shape
              ),
              action=b_actions[minibatch],
          )
...
if __name__ == "__main__":
  ...
  agent = pufferlib.frameworks.cleanrl.make_policy(
      Agent, recurrent_args=[128, 256], recurrent_kwargs={"num_layers": 1}
  )(binding).to(device)

  trainer = CleanPuffeRL(
      binding, agent, num_agents=128, num_envs=1, num_steps=128, num_cores=1
  )
