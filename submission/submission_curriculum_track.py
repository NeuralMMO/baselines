...
from curriculum.train_helper import load_agent_model, train_on_tasks
...
def train_with_curriculum(use_elm=True):
  ...
  task_encoder.get_task_embedding(
      train_task_spec, save_to_file=CURRICULUM_FILE_PATH)
  ...
  for epoch in range(30):
    ...
    if use_elm and epoch % EPOCHS_PER_ELM_UPDATE == 9:
      ...
      new_task_spec = task_generator.evolve_tasks(
          train_task_spec, NUM_NEW_TASKS, debug=DEBUG
      )
    ...
    agent_model, train_stats = train_on_tasks(
        agent_model, CURRICULUM_FILE_PATH)
