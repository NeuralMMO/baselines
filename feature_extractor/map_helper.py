...
class MapHelper:
  ...
  def reset(self):
    ...
    self.visit_map = np.zeros(
        (self._team_size, self.map_size + 1, self.map_size + 1)
    )
  ...
  def update(self, obs: Dict[int, Any], curr_step: int):
    ...
    for ent_id, player_obs in obs.items():
      ...
      # update visit map
      self._mark_point(
          self.visit_map[self._entity_helper.agent_id_to_pos(ent_id)],
            ...
            for row_offset in range(-1, 2):
              for col_offset in range(-1, 2):
                if (
                  ...
                  self.tile_map[
                      pos[0] + row_offset, pos[1] + col_offset
                  ] = material.Ocean.index

    # update the entity map based on all team obs
    self.entity_map = (
        entity_map[0] * TEAMMATE_REPR
        + entity_map[1] * ENEMY_REPR
        + entity_map[2] * PASSIVE_REPR
        + entity_map[3] * NEUTRAL_REPR
        + entity_map[4] * HOSTILE_REPR
    )
  ...
  def extract_tile_feature(self):
    ...
    dummy_img_feat = np.zeros(
        (ModelArchitecture.TILE_NUM_CHANNELS, img_size, img_size)
    )
    imgs = []
    for member_pos in range(self._team_size):
      ...
      l, r = int(curr_pos[0] - img_size // 2), int(
          curr_pos[0] + img_size // 2 + 1
      )
      u, d = int(curr_pos[1] - img_size // 2), int(
          curr_pos[1] + img_size // 2 + 1
      )
      ...
      coord_imgs = [
          self.x_img[l:r, u:d] / self.map_size,
          self.y_img[l:r, u:d] / self.map_size,
      ]

      # NOTE: realikun also considered obstacle_img, view_img
      img = np.stack(
          [tile_img, entity_img, poison_img, fog_img, visit_img, *coord_imgs]
      )
      imgs.append(img)
  ...
  def nearby_features(self, row: int, col: int, nearby_dist=NEARBY_DIST):
    ...
    # As long as the order is fixed, agents will make sense of and use these
    # features
    near_tile_map = self.tile_map[
        row - nearby_dist // 2: row + (nearby_dist // 2 + 1),
    ...
    for i in range(nearby_dist):
      for j in range(nearby_dist):
        ...
        if (
            abs(i - nearby_dist // 2) + abs(j - nearby_dist // 2)
            <= nearby_dist // 2
        ):
          feat_arr.append(
              near_tile_map[i, j] == material.Foilage.index
          )  # food_arr
          feat_arr.append(
              near_tile_map[i, j] == material.Water.index
          )  # water_arr
          feat_arr.append(
              near_tile_map[i, j] == material.Herb.index
          )  # herb_arr
          feat_arr.append(
              near_tile_map[i, j] == material.Fish.index
          )  # fish_arr
          feat_arr.append(
              near_tile_map[i, j] in material.Impassible.indices
          )  # obstacle_arr
          feat_arr.append(
              max(0, self.poison_map[row + i, col + j]) / POISON_CLIP
          )  # poison map
  ...
  def legal_moves(self, obs):
    assert (
        self.config.PROVIDE_ACTION_TARGETS
    ), "config.PROVIDE_ACTION_TARGETS must be set True"
    moves = np.zeros(
        (self._team_size, len(nmmo.action.Direction.edges)), dtype=np.float32
    )
    for member_pos in range(self._team_size):
      ent_id = self._entity_helper.pos_to_agent_id(member_pos)
      if ent_id in obs:
        moves[member_pos] = obs[ent_id]["ActionTargets"][nmmo.action.Move][
  ...
  def _get_init_tile_map(self):
    ...
    arr[map_left + 1: map_right, map_left + 1: map_right] = (
        max(material.All.indices) + 1
    )
    return arr
  ...
  def _get_init_poison_map(self):
    ...
    arr[
        center - safe: center + safe + 1, center - safe: center + safe + 1
    ] = -np.inf
  ...
  def _mark_point(self, arr_2d, index_arr, value, clip=False):
    arr_2d[index_arr[:, 0], index_arr[:, 1]] = (
        np.clip(value, 0.0, 1.0) if clip else value
    )
