...
class FeatureExtractor:
  def __init__(
      ...
      teams,
      team_id: int,
      config: nmmo.config.AllGameSystems,
  ):
    ...
    self.market_helper = MarketHelper(
        config, self.entity_helper, self.item_helper)
  ...
  def __call__(self, obs):
    ...
    state = {
        ...
        # enemy dim: (team_size, ENTITY_NUM_ENEMIES_CONSIDERED,
        # ENTITY_NUM_FEATURES)
        ...
        "reset": np.array(
            [self.game_state.curr_step == 0]
        ),  # for resetting RNN hidden,
    }
  ...
  def _make_legal_moves(self, obs):
    ...
    # attack
    legal_moves["style"] = np.ones(
        (self.team_size, ModelArchitecture.ACTION_NUM_DIM["style"]),
        dtype=np.float32,
    )
    ...
    if self._config.ITEM_SYSTEM_ENABLED:
      legal_moves["use"] = self.item_helper.legal_inventory(
          obs, nmmo.action.Use)
      legal_moves["destroy"] = self.item_helper.legal_inventory(
          obs, nmmo.action.Destroy
      )

    if self._config.EXCHANGE_SYSTEM_ENABLED:
      legal_moves["sell"] = self.item_helper.legal_inventory(
          obs, nmmo.action.Sell
      )
      # legal_moves["buy"] = self.market_helper.legal_buy(obs)

    # TODO: give, give-gold
    #   give-gold and sell should benefit from a continuous price policy head
    # rather than a discrete action head (e.g., one-hot encoding out of 100
    # discrete prices)
  ...
  def translate_actions(self, actions):
    actions_dict = {}
    for idx, a in enumerate(ModelArchitecture.ACTION_NUM_DIM.keys()):
      actions_dict[a] = [actions[idx]]
    actions = actions_dict

    # save actions to game_state.prev_atns, assuming this fn is called at the
    # step
    ...
    for member_pos in range(self.team_size):
      ...
      trans_actions[member_pos] = {
          nmmo.action.Move: {
              nmmo.action.Direction: nmmo.action.Direction.edges[
                  actions["move"][member_pos]
              ]
          }
      }
      ...
      if "target" in actions:
        target_id = self.entity_helper.set_attack_target(
            member_pos, actions["target"][member_pos]
        )
        if target_id != 0:
          trans_actions[member_pos][nmmo.action.Attack] = {
              nmmo.action.Target: target_id,
              nmmo.action.Style: nmmo.action.Style.edges[
                  actions["style"][member_pos]
              ],
          }
      ...
      if self._config.EXCHANGE_SYSTEM_ENABLED and "sell" in actions:
        ...
        inv_idx = (
            self.item_helper.force_sell_idx[member_pos]
        ...
        if self.item_helper.in_inventory(member_pos, inv_idx):
          trans_actions[member_pos][nmmo.action.Sell] = {
              nmmo.action.InventoryItem: inv_idx,
              nmmo.action.Price: self.item_helper.get_price(
                  member_pos, inv_idx
              ),
          }
