...
ITEM_TO_PROF_LEVEL = {
    ...
    Item.Spear.ITEM_TYPE_ID: "melee_level",
    Item.Bow.ITEM_TYPE_ID: "range_level",
    Item.Wand.ITEM_TYPE_ID: "mage_level",
    Item.Whetstone.ITEM_TYPE_ID: "melee_level",
    Item.Arrow.ITEM_TYPE_ID: "range_level",
    Item.Runes.ITEM_TYPE_ID: "mage_level",
    Item.Rod.ITEM_TYPE_ID: "fishing_level",
    Item.Gloves.ITEM_TYPE_ID: "herbalism_level",
    Item.Pickaxe.ITEM_TYPE_ID: "prospecting_level",
    Item.Axe.ITEM_TYPE_ID: "carving_level",
    Item.Chisel.ITEM_TYPE_ID: "alchemy_level",
    Item.Ration.ITEM_TYPE_ID: "level",
    Item.Potion.ITEM_TYPE_ID: "level",
}
...
TOOLS = {
    ...
    Item.Axe.ITEM_TYPE_ID,
    Item.Chisel.ITEM_TYPE_ID,
}

AMMOS = {
    Item.Whetstone.ITEM_TYPE_ID,
    Item.Arrow.ITEM_TYPE_ID,
    Item.Runes.ITEM_TYPE_ID,
}

CONSUMABLES = {
    Item.Ration.ITEM_TYPE_ID,
    Item.Potion.ITEM_TYPE_ID,
}
...
class ItemHelper:
  ...
  def _reset_obs_best_force(self):
    ...
    self.best_items = {
        ...
        Item.Spear.ITEM_TYPE_ID: self.best_weapons,
        ...
        Item.Axe.ITEM_TYPE_ID: self.best_tools,
  ...
  def in_inventory(self, member_pos, inv_idx):
    ...
    if (
        ...
        or not self._config.ITEM_SYSTEM_ENABLED
        or inv_idx is None
        or inv_idx >= self._config.ITEM_INVENTORY_CAPACITY
    ):
      return False
  ...
  def legal_inventory(self, obs, action):
    assert (
        self._config.PROVIDE_ACTION_TARGETS
    ...
    targets = np.zeros(
        (self._team_size, ModelArchitecture.INVENTORY_CAPACITY + 1),
        dtype=np.float32,
    )
    ...
    for member_pos in range(self._team_size):
      ...
      if ent_id in obs:
        targets[member_pos][: ModelArchitecture.INVENTORY_CAPACITY] = obs[
            ent_id
        ]["ActionTargets"][action][nmmo.action.InventoryItem]
        ...
        if action in [nmmo.action.Sell, nmmo.action.Destroy]:
          for team_best in self.best_items.values():
            ...
            if my_best is not None:
              inv_idx = self._get_inv_idx(
                  my_best[ItemAttr["id"]], obs[ent_id]["Inventory"]
              )
  ...
  def get_price(self, member_pos, inv_idx):
    ...
    if (
        agent_id not in self._obs_inv
        or not self._config.EXCHANGE_SYSTEM_ENABLED
        or inv_idx >= self._config.ITEM_INVENTORY_CAPACITY
    ):
      return np.nan
  ...
  #########################################
  def _filter_inventory_obs(
      self, obs_inv, item_type, max_equipable_lvl=np.inf, to_sell=False
  ) -> np.ndarray:
    ...
    ):  # cannot equip too-high-level (and already listed) items
  ...
  def _evaluate_best_item(self, obs):
    for agent_id, agent_obs in obs.items():
      ...
      for item_type in eval_types:
        ...
        items = self._filter_inventory_obs(
            agent_obs["Inventory"], item_type, max_equipable_lvl=agent_level
        )
        if len(items) > 0:
          max_level = max(items[:, ItemAttr["level"]])
          curr_best = self.best_items[item_type][member_pos]
          if curr_best is None or curr_best[ItemAttr["level"]] < max_level:
  ...
  def _equip_best_item(self, obs):
    for agent_id, agent_obs in obs.items():
      ...
      for item_type in eval_types:
        ...
        if (
            self.force_use_idx[member_pos] is None
            and my_best is not None
            and (my_best[ItemAttr["equipped"]] == 0)
        ):
          inv_idx = self._get_inv_idx(
              my_best[ItemAttr["id"]], agent_obs["Inventory"]
          )
      ...
      if self.force_use_idx[member_pos] is None:
        ...
        if best_tool is not None and best_tool[ItemAttr["equipped"]] == 0:
          inv_idx = self._get_inv_idx(
              best_tool[ItemAttr["id"]], agent_obs["Inventory"]
          )
          self.force_use_idx[member_pos] = inv_idx
  ...
  def _concat_types(self, obs_inv, types):
    ...
    flt_idx = (
        np.in1d(obs_inv[:, ItemAttr["type_id"]], list(types))
        & (obs_inv[:, ItemAttr["listed_price"]] == 0)
  ...
  def _sell_type(self, member_pos, obs_inv, sell_type):
    ...
    if len(sorted_items):
      # items are sorted by level (ascending), so selling the lowest-level
      # items first
  ...
  def extract_item_feature(self):
    ...
    dummy_item_arrs = np.zeros(
        (inv_capacity, ModelArchitecture.ITEM_NUM_FEATURES))
    ...
    for member_pos in range(self._entity_helper.team_size):
      ...
      for j in range(inv_capacity):
        ...
        agent_itm_types.append(
            o[ItemAttr["type_id"]]
        )  # type is 0 if item is empty
    ...
    return np.stack(team_itm_types).astype(np.float32), np.stack(
        team_itm_arrs
    ).astype(np.float32)
  ...
  def _extract_per_item_feature(o):
    if np.sum(o) > 0:
      ...
      arr = np.array(
          [
              o[ItemAttr["level"]] / 10.0,
              o[ItemAttr["quantity"]] / 10.0,
              o[ItemAttr["melee_attack"]] / 100.0,
              o[ItemAttr["range_attack"]] / 100.0,
              o[ItemAttr["mage_attack"]] / 100.0,
              o[ItemAttr["melee_defense"]] / 40.0,
              o[ItemAttr["range_defense"]] / 40.0,
              o[ItemAttr["mage_defense"]] / 40.0,
              o[ItemAttr["health_restore"]] / 100.0,
              o[ItemAttr["listed_price"]] / 100.0,
              o[ItemAttr["equipped"]],
          ]
      )
    else:
