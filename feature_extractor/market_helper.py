...
from feature_extractor.item_helper import (
    ARMORS,
    ATK_TO_WEAPON,
    ITEM_TO_PROF_LEVEL,
    ItemHelper,
...
class MarketHelper:
  ...
  def _calculate_combat_score(self):
    ...
    for member_pos in range(self._team_size):
      ...
      if self._item.best_weapons[member_pos] is not None:
        score += (
            self._item.best_weapons[member_pos][ItemAttr["level"]]
            * WEAPON_SCORE
        )
      elif self._item.best_tools[member_pos] is not None:
        score += (
            self._item.best_tools[member_pos][ItemAttr["level"]] * ARMOR_SCORE
        )
      if self._item.best_hats[member_pos] is not None:
        score += (
            self._item.best_hats[member_pos][ItemAttr["level"]] * ARMOR_SCORE
        )
  ...
  def _calculate_restore_score(self, obs):
    ...
    for agent_id, agent_obs in obs.items():
      ...
      flt_poultice = obs_inv[:, ItemAttr["type_id"]
  ...
  def _filter_market_obs(
    ...
    flt_mkt = (
        ~np.in1d(obs_mkt[:, ItemAttr["id"]], self._item.force_buy_idx)
  ...
  def _emergency_buy_poultice(self, obs):
    ...
    for member_pos in proc_order:  # start from lowest health
      ...
      agent = self._entity_helper.agent_or_none(agent_id)
      listings = self._filter_market_obs(
          agent, obs_mkt, Item.Potion.ITEM_TYPE_ID)
      if len(listings) > 0:
        # randomly selecting a listing
        # and NOT going for only the cheapest one, because others will also
        # want it
        item_id = np.random.choice(listings[:, ItemAttr["id"]].flatten())
        self._item.force_buy_idx[member_pos] = np.argwhere(
            obs_mkt[:, ItemAttr["id"]] == item_id
        ).item()
  ...
  def _buy_weapons_armors(self, obs):
    ...
    for member_pos in proc_order:  # start from lowest restore score
      ...
      for item_type, saver in zip(arms_types, best_savers):
        ...
        curr_best_lvl = (
            saver[member_pos][ItemAttr["level"]]
        ...
        if len(listings[flt_lvl]) > 0:
          best_item = sorted(
              listings[flt_lvl], key=lambda x: x[ItemAttr["level"]]
          )[-1]
          score_delta = (
              (best_item[ItemAttr["level"]] - curr_best_lvl) * ARMOR_SCORE
      ...
      if max(enhancements) > 0:
        ...
        self._item.force_buy_idx[member_pos] = np.argwhere(
            obs_mkt[:, ItemAttr["id"]] == to_buy[ItemAttr["id"]]
        ).item()
  ...
  def _buy_consumables(self, obs):
    ...
    for member_pos in proc_order:  # start from lowest restore score
      ...
      for cons_type, cons_target in buy_goal:
        if self._item.force_buy_idx[
            member_pos
        ]:  # already have something to buy
          continue
        my_items = obs[agent_id]["Inventory"][:, ItemAttr["type_id"]]
        if (
            sum(my_items == cons_type) >= cons_target
        ...
        listings = self._filter_market_obs(
            agent, obs_mkt, cons_type, acceptable_price
        )
        if len(listings) > 0:
          min_price = min(listings[:, ItemAttr["listed_price"]])
          if min_price < acceptable_price:
            listings = self._filter_market_obs(
                agent, obs_mkt, cons_type, min_price
            )
