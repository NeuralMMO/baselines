...
task_spec.append(
    ("team", StayAlive, {"target": "my_team_leader"},
     {"task_cls": OngoingTask})
)
...
for reward_to in ["agent", "team"]:
  task_spec.append(
      (reward_to, OccupyTile, {"row": 80, "col": 80})
  )  # TODO: get config
...
for skill in SKILLS:
  for level in LEVEL_GOAL:
    ...
    task_spec.append(
        ("agent", AttainSkill, {
         "skill": skill, "level": level, "num_agent": 1})
    )
...
for item in EQUIP_ITEM:
  for level in LEVEL_GOAL:
    # agent task
    task_spec.append(
        ("agent", EquipItem, {"item": item, "level": level, "num_agent": 1})
    )
