...
class HeuristicTaskGenerator:
  ...
  def generate_task_definitions(
    ...
    gen.add_pred_spec(
        p.AllDead, []
    )  # TODO: should this predicate have a "target" param?
    ...
    gen.add_pred_spec(
        p.DistanceTraveled, [N]
    )  # TODO: should this param be COORDINATE_CONSTRAINT?
    ...
    gen.add_pred_spec(
        p.InventorySpaceGE, [N]
    )  # TODO: should this param be INVENTORY_CONSTRAINT?
    ...
    task_infos = []
    for _ in range(n):
      task_info = gen.sample(
          max_clauses=max_clauses, max_clause_size=max_clause_size, not_p=not_p
      )
    ...
    def new_task(gs, subject):
      ...
      for clause_info in task_info:
        ...
        for pred_info in clause_info:
          ...
          if pred_info["not"] == True:
            predicate = t.NOT(predicate)
...
if __name__ == "__main__":
  ...
  parser.add_argument("-n", "--n", default=5,
