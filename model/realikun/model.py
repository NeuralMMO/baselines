...
class ModelArchitecture:
    ...
    ACTION_NUM_DIM = sort_dict_by_key(
        {
            ...
            "target": ENTITY_NUM_NPCS_CONSIDERED
            + ENTITY_NUM_ENEMIES_CONSIDERED
            + 1,  # for no attack
    ...
    SELF_NUM_FEATURES = (
        TEAM_NUM_FEATURES + GAME_NUM_FEATURES + sum(ACTION_NUM_DIM.values())
    )
...
class PrevActionEncoder(nn.Module):
    def __init__(self, n_legal, n_embed):
        ...
        self.embedding = nn.ModuleDict(
            {name: nn.Embedding(n_input, n_embed) for name, n_input in n_legal.items()}
        )
...
class SelfEncoder(nn.Module):
    def __init__(self, in_img_ch, in_img_size, n_self_feat, n_legal, n_self_hidden):
        ...
        mlp_input_size = (
            self.img_net.h_size
            + n_self_feat  # 1024
            + prev_act_hidden_size  # 318
            + item_hidden_size  # 64  # 128
        )
...
class EntityEncoder(nn.Module):
    def __init__(self, entity_type, n_entity_feat, n_attn_hidden):
        ...
        self.net = MLPEncoder(n_attn_hidden + n_entity_feat, n_hiddens=[n_attn_hidden])
...
class InteractionBlock(nn.Module):
    def __init__(self, n_attn_hidden):
        ...
        encoder_block = nn.TransformerEncoderLayer(
            d_model=n_attn_hidden, nhead=2, dim_feedforward=512, dropout=0.0
        )
...
class MemoryBlock(nn.Module):
    def __init__(self, n_attn_hidden, n_lstm_hidden, num_layers):
        ...
        assert (
            n_attn_hidden == 256
        ), "n_attn_hidden must be 256 for compatibility with CleanRL"
        assert (
            n_lstm_hidden == 512
        ), "n_lstm_hidden must be 512 for compatibility with CleanRL"
    ...
    def forward(self, x, state):
        ...
        if state is None:
            shape = (
                self.num_layers,
                teams * ModelArchitecture.NUM_PLAYERS_PER_TEAM,
                self.hidden_size,
            )
            state = [
        ...
        hxs = hxs.view(
            -1, teams * ModelArchitecture.NUM_PLAYERS_PER_TEAM, self.hidden_size
        )
        cxs = cxs.view(
            -1, teams * ModelArchitecture.NUM_PLAYERS_PER_TEAM, self.hidden_size
        )
