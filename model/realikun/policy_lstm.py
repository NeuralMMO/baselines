...
class RealikunPolicyLSTM(pufferlib.models.Policy):
  def __init__(self, binding, input_size=512, hidden_size=256):
    ...
    self.proj_fc = torch.nn.Linear(ModelArchitecture.SELF_EMBED, input_size)
  ...
  def encode_observations(self, env_outputs):
    ...
    # h_inter = self.interact_fc(h_self.view(batch_size, -1)).view(batch_size, num_agents, -1)
    h_inter = self.proj_fc(h_inter)
  ...
  def decode_actions(self, hidden, lookup, concat=True):
    ...
    hidden = hidden.view(batch_size, 8, -1)
