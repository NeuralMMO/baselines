...
class Policy(pufferlib.models.Policy):
  ...
  def encode_observations(self, env_outputs):
    ...
    tile = tile.view(agents, tiles, features*embed).transpose(1,
                                                              2).view(agents, features*embed, 15, 15)
    ...
    mask = mask.int()
    row_indices = torch.where(mask.any(dim=1), mask.argmax(
    ...
    my_agent_embeddings = agent_embeddings[torch.arange(
        agents.shape[0]), row_indices]
