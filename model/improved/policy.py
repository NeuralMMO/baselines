...
class ImprovedPolicy(pufferlib.models.Policy):
  def __init__(self, binding, input_size=256, hidden_size=256, output_size=256):
    ...
    self.entity_offset = torch.tensor([i*256 for i in range(3, 26)])
  ...
  def encode_observations(self, env_outputs):
    ...
    tile = tile.view(agents, tiles, features*embed)\
      .transpose(1, 2).view(agents, features*embed, 15, 15)
  ...
  def create_policy(num_lstm_layers=1):
    ImprovedPolicy.INPUT_SIZE = 128
    if num_lstm_layers == 0:
        ImprovedPolicy.HIDDEN_SIZE = 128
        policy = pufferlib.frameworks.cleanrl.make_policy(
        ImprovedPolicy, recurrent_kwargs={'num_layers': 0})
    else:
        ImprovedPolicy.HIDDEN_SIZE = 256
        policy = pufferlib.frameworks.cleanrl.make_policy(
        ImprovedPolicy, recurrent_args=[ImprovedPolicy.INPUT_SIZE, ImprovedPolicy.HIDDEN_SIZE],
